 ...
 * Data: 04/06/2025
 *
 * Código base para projeto IoT utilizando protocolo MQTT, a ser adaptado para
 * controlar cortinas com base na luz ambiente medida por um sensor LDR, com ajustes
 * automáticos e manuais em níveis intermediários (0% a 100% de abertura). Utiliza
 * a placa BitDogLab com Wi-Fi, LDR, LED RGB, matriz de LEDs WS2812, botões, joystick,
 * display OLED e buzzer. Este código será a base para desenvolvimento no repositório
 * GitHub, mantendo a funcionalidade original de publicar temperatura e controlar LED.
 *
...
static float read_ldr() {
    adc_select_input(2); // Seleciona ADC2 (GPIO 28)
    float soma = 0.0f;
    for (int i = 0; i < 100; i++) { // 100 leituras para estabilidade
        soma += adc_read(); // Lê valor do ADC (0 a 4095)
        sleep_ms(1); // Atraso de 1ms entre leituras
    }
    float media = soma / 100.0f; // Calcula média para reduzir ruído

    // Define valores de referência do ADC (ajustar com testes reais)
    float adc_min = 100.0f;  // Valor do ADC em pouca luz (quarto escuro)
    float adc_max = 4000.0f; // Valor do ADC em muita luz (quarto claro)

    // Mapeia o valor do ADC para 0-100% (muita luz = 100%, pouca luz = 0%)
    float light = 100.0f * (adc_max - media) / (adc_max - adc_min);
    
    // Limita a saída entre 0 e 100%
    light = light < 0.0f ? 0.0f : light > 100.0f ? 100.0f : light;

    return light; // Retorna intensidade de luz em porcentagem
}

static void publish_light(MQTT_CLIENT_DATA_T *state) {
    static float old_light = -1.0f;
    const char *light_key = full_topic(state, "/casa/luz");
